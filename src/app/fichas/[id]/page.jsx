  ...
  const salvarFicha = () => {
    ...
    if (fichasSalvas) {
      ...
      toast.success("Ficha salva com sucesso!", {
        ...
        autoClose: 2500,          // Fecha sozinho após 3 segundos
        theme: resolvedTheme === 'dark' ? 'dark' : 'light',
      });
    }
  };

  // ... (As funções de atualização permanecem exatamente iguais)
    ...
    <main className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 text-fd-primary relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-8">
          ...
          {/* EXEMPLO 1: CAMPOS SIMPLES */}
          <section className="bg-fd-background/80 p-6 rounded-xl border border-fd-primary/20 shadow-sm backdrop-blur-sm">
            <h2 className="text-xl font-bold mb-4 border-b border-fd-primary/20 pb-2 data-[active=true]:text-fd-primary">Informações Básicas</h2>
            <div className="flex flex-col gap-4">
              <label className="flex flex-col gap-1">
                <span className="font-semibold text-fd-primary/80">Nome</span>
                {modoEdicao ? (
                  <input 
                    className="bg-transparent border border-fd-primary/30 text-fd-primary data-[active=true]:text-fd-primary rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow"
                    value={f.nome} 
                    onChange={(e) => atualizarCampoBase('nome', e.target.value)} 
                  />
                ) : (
                  <span className="bg-fd-background/80 text-fd-primary px-3 py-2 rounded-md border border-fd-primary/10">{f.nome || 'Não definido'}</span>
                )}
              </label>
              ...
              <label className="flex flex-col gap-1">
                ...
                {modoEdicao ? (
                  <input 
                    className="bg-transparent border border-fd-primary/30 text-fd-primary data-[active=true]:text-fd-primary rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow"
                    value={f.vocacao} 
                    onChange={(e) => atualizarCampoBase('vocacao', e.target.value)} 
                  />
                ) : (
                  <span className="bg-fd-background/80 text-fd-primary px-3 py-2 rounded-md border border-fd-primary/10">{f.vocacao || 'Não definida'}</span>
                )}
              </label>
            </div>
          </section>
      ...
      {/* 3. Componente que exibe os toasts (pode ficar no final da main) */}
      <ToastContainer />
    </main>
  );
}