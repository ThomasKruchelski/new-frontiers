...
export default function BonusEImplantes() {
  ...
  return (
    <section className="bg-fd-background/80 p-6 rounded-xl border border-fd-primary/20 shadow-sm backdrop-blur-sm">
      ...
      {f.bonusEImplantes.length === 0 && !modoEdicao && (
        <p className="text-fd-primary/50 italic">Nenhu Bônus adicionado.</p>
      )}

      <ul className="flex flex-col gap-3 mb-4">
        {f.bonusEImplantes.map((bonus, index) => (
          <li key={index} className="flex flex-col border border-fd-primary/20 rounded-md overflow-hidden">
            ...
            {modoEdicao ? (
              ...
              <div className="p-3 flex flex-col gap-2 bg-fd-background/50">
                <div className="flex gap-2 items-start">
                  <input
                    ...
                    value={bonus.nome || ''}
                    onChange={(e) => atualizarObjetoArray('bonusEImplantes', index, 'nome', e.target.value)}
                  />
                  <button
                    onClick={() => removerObjetoArray('bonusEImplantes', index)}
                ...
                <textarea
                  ...
                  onChange={(e) => atualizarObjetoArray('bonusEImplantes', index, 'descricao', e.target.value)}
                />
                <input
                  placeholder="Tipo do Bônus"
                  className="flex-1 bg-transparent border border-fd-primary/30 text-fd-primary rounded-md px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500 font-semibold"
                  value={bonus.tipo || ''}
                  onChange={(e) => atualizarObjetoArray('bonusEImplantes', index, 'tipo', e.target.value)}
                />
              </div>
            ) : (
              ...
              <>
                ...
                >
                  <span className="font-semibold text-fd-primary">{bonus.nome || 'Bônus sem nome'}</span>

                  {/* Ícone de setinha que gira quando aberto */}
                  <div className='flex gap-2'>
                    <span className="text-fd-primary/60 text-sm">{bonus.tipo}</span>
                    <svg
                ...
                {indiceAberto === index && (
                  <div className="p-3 border-t border-fd-primary/10 text-fd-primary/80 bg-fd-background/40">
                    {bonus.descricao ? (
                      <p className="whitespace-pre-wrap">{bonus.descricao}</p>
                    ) : (
      ...
      {modoEdicao && (
        <button
          onClick={() => adicionarObjetoArray('bonusEImplantes', { nome: "", tipo: "", descricao: "" })}
